...
class VoteRepository {
     ...
     */
    static async findAllWithVotes(currentUserId, limit = 100, offset = 0, savedOnly = false) {
     ...
     */
    static async findCategories(currentUserId) {
        const result = await (0, db_js_1.query)(`SELECT DISTINCT jsonb_array_elements_text(t.config->'tags') AS tag
       FROM templates t
       WHERE (t.user_id IS NULL OR t.user_id = $1)
         AND t.config->'tags' IS NOT NULL
         AND jsonb_array_length(t.config->'tags') > 0
       ORDER BY tag`, [currentUserId || null]);
